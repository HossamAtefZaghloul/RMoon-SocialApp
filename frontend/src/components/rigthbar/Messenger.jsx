...
export default function Messenger({ friendChat, handleToggleFalse }) {
  ...
  useEffect(() => {
    // Join the room when the component mounts (if needed)
    ...
    <div className="w-[300px] h-[350px] top-[50px] bg-[#18191A] border border-gray-700 rounded-t-lg shadow-lg flex flex-col">
      <div className="flex justify-between items-center p-3 border-b border-gray-800">
        <div className="flex items-center gap-3">
          <img className="w-[35px] h-[35px] rounded-full" src={server + friendChat.image} alt="Profile" />
          <h3 className="font-semibold text-red-500">{friendChat.username}</h3>
        </div>
        <button onClick={handleToggleFalse} className="text-red-700 hover:text-red-900 focus:outline-none">
          <X className="h-4 w-4" />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-3" ref={scrollAreaRef}>
        {messages.map((msg) => (
          ...
          >
            <span className="text-gray-500 text-[11px]">{formatTimestampToDayAndTime(msg.timestamp)}</span>
            <div className="flex items-center justify-end gap-2"></div>
            <span
              className={`inline-block p-2 rounded-lg ${
                msg.sender === sender ? "bg-red-900 text-white" : "bg-gray-800 text-white"
              }`}
            >
              {msg.content}
            </span>
      ...
      <form className="p-3 border-t border-gray-700 flex" onSubmit={handleSendMessage}>
        <input
          type="text"
          value={message}
